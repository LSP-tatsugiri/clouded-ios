...
`docs/step-6-plan.md` is the iOS capture app (built on the Mac; read it
before touching `ios/`); `docs/step-7-plan.md` is sharing and the friend
skill pool (built on the PC). Both are done and merged.
...
- `ios/` — the iOS capture app and its Share Extension (SwiftUI, iOS 17,
  `supabase-swift` via SPM, project generated by XcodeGen from
  `project.yml`). Built last, because what an entry stores depends on what
