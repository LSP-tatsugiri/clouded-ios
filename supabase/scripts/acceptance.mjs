...
// idea to a group, checks who sees what (RLS, the idea-media bucket, profiles,
// invites, leaving), and cleans up; its one source_url probe inserts an idea,
// so it costs one extraction (~$0.01). list prints the reference for the web
// list (docs/step-5-plan.md Phase C): the sorted order and the leverage top 10
// for --user's profile, from the same rows and the same distance.js the
// browser imports. seed and rerun spend API credits; rls spends one call.
...
async function rls() {
  ...
  let B, C, D, gid, gid2, ideaId, otherId, probeId, aName;
  ...
  try {
    ...
    // Step 7 below invites C into the group and un-shares the idea, so the
    // storage checks (C outside, idea shared) ran first.
